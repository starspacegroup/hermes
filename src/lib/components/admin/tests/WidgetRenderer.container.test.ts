...
describe('WidgetRenderer - Container Display Modes', () => {
  const createContainerWidget = (config: Record<string, unknown>): PageWidget => ({
    id: 'test-container',
    page_id: 'test-page',
    type: 'container',
    position: 0,
  ...
  describe('Flexbox Display Mode', () => {
    it('renders container with flex display mode', () => {
      ...
      const containerElement = container.querySelector('.container-widget') as HTMLElement;
    ...
    it('applies responsive flex direction', () => {
      ...
      const { container: mobileContainer } = render(WidgetRenderer, {
        props: {
          widget,
      ...
      const mobileElement = mobileContainer.querySelector('.container-widget') as HTMLElement;
      expect(mobileElement.style.display).toBe('flex');
      expect(mobileElement.style.flexDirection).toBe('column');

      // Test desktop breakpoint
      const { container: desktopContainer } = render(WidgetRenderer, {
        props: {
          widget,
          currentBreakpoint: 'desktop' as Breakpoint,
          isEditable: false
      ...
      const desktopElement = desktopContainer.querySelector('.container-widget') as HTMLElement;
  ...
  describe('Grid Display Mode', () => {
    ...
    it('applies responsive grid columns', () => {
      ...
      const { container: tabletContainer } = render(WidgetRenderer, {
        props: {
          widget,
      ...
      const tabletElement = tabletContainer.querySelector('.container-widget') as HTMLElement;
